...
export default function VistaPlanVsReal() {
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      setReal(recorridosResp.data.find((r) => r.estado === 'finalizado') ?? null)
    } catch {
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      showToast('✅ Plan del día guardado — los vaqueros pueden verlo', 'exito')
      setFormAbierto(false)
      cargar()
    } catch (err) {
            ...
            <div className="rounded-2xl border border-border bg-card p-4">
              ...
              {plan && !real && (
                <p className="mb-3 text-sm text-text-secondary">
                  Los vaqueros aún no han salido — se muestra únicamente el plan.
                </p>
              )}
              {!plan && real && (
                <p className="mb-3 text-sm text-text-secondary">
