...
export default function TarjetaRecorrido({ recorrido, onClick, onEliminar }) {
  const badge =
  ...
  return (
    <div className="relative">
      {onEliminar && (
        <button
          type="button"
          onClick={() => onEliminar(recorrido)}
          className="absolute right-3 top-3 z-10 flex h-9 w-9 items-center justify-center rounded-full border-2 border-error/50 bg-card text-base text-error transition hover:bg-error/10"
          aria-label="Borrar recorrido"
        >
          🗑️
        </button>
      )}
