...
export default function Paso2Corraletas({ corraletas, paradas, color, onAgregar, onQuitarUltima }) {
  const selectedIds = new Set(paradas.map((p) => p.id))
  ...
  return (
    <div className="space-y-4">
      ...
      <MapaRecorrido
        ...
        paradasSeleccionadas={paradas}
        color={color}
        onSelectCorraleta={onAgregar}
        readOnly={false}
        height="260px"
      />
          ...
          <p className="text-sm text-text">
            {paradas.map((p, i) => (
              <span key={p.id}>
                ...
                <span>{p.nombre}</span>
                {i < paradas.length - 1 && <span className="text-text-secondary"> → </span>}
              </span>
            ))}
          </p>
        </div>
      ...
      {paradas.length === 0 && (
        <p className="text-center text-sm text-text-secondary">
          Toca los marcadores en el mapa o selecciona corraletas abajo para armar la ruta.
        </p>
      )}
      ...
      {paradas.length > 0 && (
        ...
        >
          ← Quitar última parada ({paradas[paradas.length - 1]?.nombre})
        </button>
      )}
      ...
      <div>
        <p className="mb-2 text-sm font-medium text-text-secondary">
          Selecciona las corraletas visitadas (mín. 2)
        </p>
        <div className="grid grid-cols-2 gap-2">
          {corraletas.map((c) => {
            const isSelected = selectedIds.has(c.id)
            const orden = paradas.findIndex((p) => p.id === c.id) + 1
            return (
              <button
                key={c.id}
                type="button"
                onClick={() => {
                  if (!isSelected) onAgregar(c)
                }}
