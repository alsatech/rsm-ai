...
// Nota de voz para la recepción de material — misma lógica que el grabador de Flota
// (GrabadorAudio.jsx), adaptada al tema oscuro de Inventario. UX ultra-simple:
//   - Tap en el botón rojo → empieza a grabar (timer grande, vibra si el dispositivo lo soporta).
//   - Tap otra vez → para y queda lista para enviarse junto con el resto del formulario.
//   - Si el navegador bloquea el micrófono (HTTP sin certificado, permiso negado, sin
//     hardware), aparece un botón para subir un audio ya grabado — funciona en cualquier caso.
...
function clasificarError(e) {
  ...
  if (e?.name === 'NotFoundError' || e?.name === 'OverconstrainedError' || msg.includes('not found')) {
...
function GrabadorTap({ onAudioListo, onError }) {
  ...
  const empezar = async () => {
    ...
    try {
      ...
      recorder.onstop = () => {
        ...
        const archivo = new File([blob], `nota-recepcion-${Date.now()}.${ext}`, { type: tipo })
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      {grabando && (
        <div className="flex items-center gap-2 rounded-full bg-error/10 px-4 py-1.5">
          <span className="inline-block h-3 w-3 animate-pulse rounded-full bg-error" />
          <span className="font-mono text-2xl font-bold tabular-nums text-error">{fmt(segundos)}</span>
        </div>
      )}
      ...
      <button
        ...
        disabled={preparando}
        aria-label={grabando ? 'Toca para parar y guardar' : 'Toca para grabar audio'}
        style={{ width: '80px', height: '80px' }}
        className={`relative flex items-center justify-center rounded-full text-3xl text-white shadow-xl transition active:scale-95 disabled:cursor-wait disabled:opacity-70 ${
          grabando ? 'animate-pulse bg-error ring-4 ring-error/30' : 'bg-error hover:brightness-110'
        }`}
      >
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <button
        ...
        style={{ width: '80px', height: '80px' }}
        className="flex items-center justify-center rounded-full bg-accent text-3xl text-highlight shadow-xl transition hover:brightness-110 active:scale-95"
      >
        📎
      </button>
      <input
      ...
      />
      <p className="text-center text-sm font-bold text-text">Toca para subir un audio</p>
    </div>
  )
}

export default function GrabadorAudio({ onAudioListo }) {
  const [soportaGrabar] = useState(() =>
    Boolean(
  ...
  return (
    <div className="flex w-full flex-col items-center gap-2 rounded-xl border border-border bg-bg p-4">
      {mostrarFallback ? (
      ...
      {soportaGrabar && (
        <button
          ...
          className="text-[11px] font-semibold text-highlight underline"
        >
